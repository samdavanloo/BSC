...
# -*- coding: utf-8 -*-
"""
Created on Sun Oct 17 20:38:10 2021
...
iter_j_max = int(np.ceil(np.sqrt(iter_max)))
iter_k_max = iter_j_max

batch_grad_B = batch_grad
batch_val_B = batch_grad_B**2

batch_grad_S = int(np.ceil(np.sqrt(batch_grad_B)))
batch_val_S = batch_grad_S**2

iter_max = iter_j_max*iter_k_max
...
for repeat in range(exp_num):
    ...
    x_history, x_tilde_history, oracle_innervalue, oracle_innergrad = RoS_VR(A, b, tau, lmbda, batch_grad_B,batch_grad_S,
